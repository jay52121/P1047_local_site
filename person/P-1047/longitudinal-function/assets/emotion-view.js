...
function evidenceHtml(definition, leftWeek, rightWeek) {
  return definition.evidence.map(([label, field, unit, higher]) => {
...
export function mountEmotionView({ main, weeks, repository, clock, personId }) {
  ...
  function update() {
    ...
    document.getElementById("emotionDetailTitle").textContent = `${definition.label} · 变化依据`; document.getElementById("emotionDetailExplain").textContent = definition.description; document.getElementById("emotionEvidenceList").innerHTML = evidenceHtml(definition, leftWeek, rightWeek);
